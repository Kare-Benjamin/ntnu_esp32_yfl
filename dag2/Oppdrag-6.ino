...
void loop() 
{
  ...
  if(verdiLDR < terskelLDR)
  {
    ...
    {
      verdiLDR = analogRead(pinLDR);
      //delay(1000); //Dette kan brukes til å jevne ut små bevegelser fra sensoren, og forholde døren åpen. Fjern // før "delay" for å bruke denne linjen.
    }
    delay(3500);
    myservo.write(posLukket);             // Lukk bommen
    delay(100);
  ...
  // Styring av rele. Relet er aktivt høy (men kan også være lav for andre typer Relé...). Strømtrekk 0,6 mA
  if(antall == 0)
  {
